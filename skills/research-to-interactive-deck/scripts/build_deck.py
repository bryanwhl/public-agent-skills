...
def render(data: dict, template: str) -> str:
    ...
    for section in sections:
        ...
        details = f'<details><summary>Go deeper</summary><p>{esc(deep_dive)}</p></details>' if deep_dive else ""
    ...
    if checks:
        ...
        rendered_sections.append(
            f'<section class="slide" id="knowledge-check"><p class="eyebrow">Retrieval practice</p>'
            f'<h2>Check your understanding</h2>{questions}</section>'
        )
    ...
    for source in sources:
        ...
        source_items.append(
            ...
            f'{esc(require(source, "publisher"))}; published/updated {esc(source.get("published", "n.d."))}; '
            f'accessed {esc(require(source, "accessed"))}.</li>'
        )
    sources_html = (
        '<section class="slide sources" id="sources"><p class="eyebrow">Evidence</p>'
        f'<h2>Sources and methodology</h2><ol>{"".join(source_items)}</ol></section>'
    )
